...
- Step 2: `domain/models.py`, `domain/specs.py`, `domain/schema_checklist.md`, `contracts.py` (include `contracts_version`), `examples/*.json`, `__init__.py`
- Step 3: split decision layer under `application/` (for example `input_validation.py`, `decision_logic.py`, `output_mapping.py`) + optional `service.py` facade + tests + `application/FILE_MAP.md`
- Step 4: `application/orchestrator.py`
- Step 5: `adapters/<interface>/<tool>/...` + `__init__.py` + `io.py` + `presenter.py` + `RUNTIME_DEPENDENCIES.md` + `assets/` + adapter smoke tests + `UI_CONTRACT.md`
