# Docs Index

- `README.md`: repo overview and run instructions.
- `AGENTS.md`: hard rules for LLM agents and folder structure.
- `docs/OVERVIEW.md`: architecture overview.
- `docs/integration/CHECKLIST.md`: required steps for a new tool.
- `docs/integration/ADAPTER_UI_KIT.md`: shared Flask UI kit.
- `docs/agent/INTERFACE_SPEC.md`: agent calling conventions.
- `docs/soft/WORKFLOW.md`: deterministic Soft Code workflow.
- `docs/soft/01_decision_list.md`: Step 1.
- `docs/soft/02_data_shapes.md`: Step 2.
- `docs/soft/03_decision_functions.md`: Step 3.
- `docs/soft/04_orchestrator.md`: Step 4.
- `docs/soft/05_adapters_and_io.md`: Step 5.
- `docs/soft/06_verification.md`: Step 6.
- `docs/soft/MIGRATION_WORKFLOW.md`: existing tool migration flow.
- `docs/manifesto/BUILDING_SOFTWARE_THAT_STAYS_SOFT.md`: manifesto.
- `START_HERE.md`: entry point.
